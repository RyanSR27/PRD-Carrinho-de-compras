...
class Carrinho
{
    ...
    {
        foreach ($this->product as $index => $produto) {
            if ($produto['id'] === $id) {
                if ($quantidade <= $produto['estoque']) {
                    // Posso adicionar, quantidade suficiente

                    // Diminui estoque disponível
                    $this->product[$index]['estoque'] -= $quantidade;
    ...
    public function removerItemDoCarrinho(int $id)
    {
        ...
        foreach ($this->product as $index => $produto) {
            if ($produto['id'] === $id) {
                $this->product[$index]['estoque'] += $quantidadeParaRestaurar;
                break;
            }
        }
    }
    ...
    private function atualizarTotais()
    {
        foreach ($this->items as $index => $item) {
            ...
            if ($this->discount > 0) {
                $desconto = $subtotal * $this->discount;
            }
